...
async function main() {
  ...
  if (!wantOn && !wantOff) {
    console.log("Usage:");
    console.log("  node scripts/tl_staking_pause_schedule.js --on   # schedule staking pause");
    console.log("  node scripts/tl_staking_pause_schedule.js --off  # schedule staking unpause");
    process.exit(1);
  }
  const targetPaused = wantOn; // true = pause staking, false = unpause staking
  ...
  console.log("=== SCHEDULE L2", targetPaused ? "STAKING PAUSE" : "STAKING UNPAUSE", "(via Timelock) ===");
  ...
  // Optional: read stakingPaused if you expose it anywhere.
  // If not exposed, skip state read and just schedule.
  console.log("\n📊 L2 State:");
  console.log("  stakingPaused(): (not read here — not exposed on minimal reader by default)");
  console.log("  target         :", targetPaused);

  // Build L2 call: l2SetStakingPause(bool)
  ...
  // Quote “configured funding” from governance (caps)
  ...
  if (balance < requiredValue) {
    throw new Error(`❌ Insufficient balance. Need ${ethers.formatEther(requiredValue)} ETH, have ${ethers.formatEther(balance)} ETH`);
  }
  ...
  console.log(`\n📅 Scheduling ${targetPaused ? "STAKING PAUSE" : "STAKING UNPAUSE"} operation via Timelock...`);
  ...
  const rc = await scheduleTx.wait();
  console.log("✅ Scheduled in block:", rc.blockNumber);
  ...
  console.log(`  node scripts/tl_staking_pause_execute.js ${operationId} ${salt} ${targetPaused ? "--on" : "--off"}`);
}
