...
async function main() {
  ...
  const PK1           = (process.env.MS_EOA1_PK || "").trim();   // owner 1 of mini
  const PK2           = (process.env.MS_EOA2_PK || "").trim();   // owner 2 of mini
  ...
  if (![TL, MINI, L2TOK, PROJECT_WALLET].every(isAddr)) {
    throw new Error("One of TL/MINI/L2_TOKEN_PROXY/PROJECT_WALLET invalid");
  }
  ...
  console.log("=== CONFIGURE TRUSTED API VIA L2 TIMELOCK ===");
  console.log("Network        : arbitrumSepolia");
  console.log("Timelock       :", TL);
  console.log("Mini           :", MINI);
  console.log("L2 token       :", L2TOK);
  console.log("Owner1 (w1)    :", await w1.getAddress());
  console.log("Owner2 (w2)    :", await w2.getAddress());
  ...
  // --- Token role check: TL must have PARAMETER_ADMIN_ROLE ---
  const DEFAULT_ADMIN_ROLE = await tok.DEFAULT_ADMIN_ROLE();
  const roleIds = await tok.getRoleIds();
  // [PAUSER, MINTER, SIGNER, PARAMETER_ADMIN, EMERGENCY_ADMIN, UPGRADER, API_SIGNER]
  const PARAMETER_ADMIN_ROLE = roleIds[3];

  const tlIsParamAdmin = await tok.hasRole(PARAMETER_ADMIN_ROLE, TL);
  ...
  if (!tlIsParamAdmin) {
    throw new Error(
      ...
      "Ensure you ran grant_param_admin_to_l2_timelock.js successfully."
    );
  }

  // --- Build call data for setTrustedAPI(PROJECT_WALLET, true) ---
  const tokIface = new ethers.Interface(TOKEN_ABI);
  const data = tokIface.encodeFunctionData("setTrustedAPI", [
    PROJECT_WALLET,
    true,
  ]);
  ...
  const salt = ethers.keccak256(
    ethers.toUtf8Bytes(`TRUSTED_API:${PROJECT_WALLET.toLowerCase()}`)
  );
  ...
  // --- Dry-run: simulate call as TL to token.setTrustedAPI ---
  console.log("\n[Dryrun] eth_call from TL to token.setTrustedAPI…");
  const dry = await ethCallFrom(l2, TL, L2TOK, data);
  if (!dry.ok) {
    console.error("Dryrun REVERTED. Reason:", dry.error);
    console.error("⚠️  Aborting; fix roles / preconditions then retry.");
    return;
  } else {
    console.log("Dryrun OK – call would succeed when executed by TL.");
  }

  // --- SCHEDULE via Mini (if not already scheduled) ---
  const already = await tl.isOperation(opId);
  if (!already) {
    console.log("\n[Schedule] setTrustedAPI via MiniMultisig");
  ...
  if (await tl.isOperationDone(opId)) {
    console.log("ℹ️ Already executed; nothing left to do.");
    return;
  }

  console.log("\n[Execute] setTrustedAPI via Timelock");
  try {
    if (executorOpen) {
  ...
  console.log(`✅ setTrustedAPI(${PROJECT_WALLET}, true) applied via TL.`);
}

main().catch((e) => {
  console.error("❌ configure_trusted_api_via_l2_timelock failed:", e);
  process.exit(1);
});
