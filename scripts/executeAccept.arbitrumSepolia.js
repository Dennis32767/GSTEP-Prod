// scripts/executeAccept.arbitrumSepolia.js
// Usage examples:
//
...
//    node scripts/executeAccept.arbitrumSepolia.js --network arbitrumSepolia --file deployments/arbitrumSepolia-latest.json --step both --wait
//
...
async function tryMultisigCall({ mini, pickedAbi, w1, w2, to, value, data }) {
  ...
  if (mini.interface.getFunction("propose(address,uint256,bytes)") &&
    ...
    try {
      ...
      const tx1 = await miniA.propose(to, value, data, { gasLimit: 1_500_000 }); // Increased for Arbitrum
      ...
      const tx2 = await mini.connect(w2).approve(txId, { gasLimit: 500_000 }); // Increased for Arbitrum
      await tx2.wait();
      console.log("Second owner approved");
      
      // Execute (can be done by any owner)
      await delay(2000);
      const tx3 = await miniA.execute(txId, { gasLimit: 800_000 }); // Increased for Arbitrum
      await tx3.wait();
  ...
  if (mini.interface.getFunction("execute(address,uint256,bytes,uint256,bytes,bytes)") && nonce !== null) {
    ...
    try {
      const tx = await miniA.execute(to, value, data, nonce, sigA, sigB, { gasLimit: 1_500_000 }); // Increased for Arbitrum
      await tx.wait();
      return;
    } catch (e) {
  ...
  if (mini.interface.getFunction("execute(address,bytes,uint256,bytes,bytes)") && nonce !== null) {
    ...
    const sigB = await w2.signTypedData(domain, types, message);
    try {
      const tx = await miniA.execute(to, data, nonce, sigA, sigB, { gasLimit: 1_500_000 }); // Increased for Arbitrum
      await tx.wait();
      return;
    } catch (e) {
  ...
  if (mini.interface.getFunction("submit(address,uint256,bytes)") &&
    ...
    try {
      const tx1 = await miniA.submit(to, value, data, { gasLimit: 1_500_000 }); // Increased for Arbitrum
      const rc1 = await tx1.wait();
      ...
      await (mini.connect(w2)).confirm(txId, { gasLimit: 500_000 }); // Increased for Arbitrum
      await miniA.execute(txId, { gasLimit: 800_000 }); // Increased for Arbitrum
      return;
    } catch (e) {
  ...
  if (mini.interface.getFunction("forward(address,uint256,bytes)")) {
    ...
    try {
      const tx = await miniA.forward(to, value, data, { gasLimit: 1_500_000 }); // Increased for Arbitrum
      await tx.wait();
      return;
    } catch (e) {
